...
class TRAVEL_ITINERARY_Api extends Default_Api{

	
	protected $action = API_HIT_TRAVEL_ITINERARY_SEARCH;
	
	/**
	* main API
	*
	*/
	function __construct($action=API_HIT_TRAVEL_ITINERARY_SEARCH,$slim=null,$showheader=false,$showres=true)
	{
		//init
		$this->action = $action;
	...
	public function hit($act=null)
	{
		...
		try
		{
			...
			switch($this->action)
			{
				case API_HIT_TRAVEL_ITINERARY_SEARCH:
	...
	protected function do_entry_search()
	{
			...
			//get params
			$port_code  = trim($_REQUEST['port_code']);
	...
	protected function get_page_list($pdata=array())
	{
		...
		//filter
		$xwhere = array();
		if(strlen($pdata['port_code']))
		{
			$t        = addslashes($pdata['port_code']);
			$xwhere[] = " AND p.port_code LIKE '%$t%' ";
		}	
